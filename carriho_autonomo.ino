...
void setup() 
{
  ...
  pinMode(trig1, OUTPUT); //definição do emissor do sensor 1
  pinMode(echo1, INPUT); //definição do receptor do sensor 1
  ...
  pinMode(LEDe, OUTPUT);
  pinMode(LEDd, OUTPUT);
  servo.attach(S); //inicialização do servo motor
  digitalWrite(LEDe, HIGH);
  digitalWrite(LEDd, HIGH);
  pinMode(MOTOR, OUTPUT); //inicialização do motor
  digitalWrite(MOTOR, HIGH);  
  servo.write (C); //definição da posição inicial do servo motor no angulo C=90
  delay(500);
}
...
void loop() 
{
  ...
  long duration1, distance1; //bloco de leitura e armazenamento dos dados do sensore ultrasonico
